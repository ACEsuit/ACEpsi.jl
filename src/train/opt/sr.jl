...
function opts!(i, OptParams::OPTPARAMS, optimizer::DirectSolver, Eloc, o::Matrix{T}, nbatch, damping::Float64, dim_ps::Int64, η::Float64, norm_constrain, γ, m) where {T}
    s = o * o' # O * O'
...
function opts!(i, OptParams::OPTPARAMSPRING, optimizer::SPRINGSolver, Eloc, o::Matrix{T}, nbatch, damping::Float64, dim_ps::Int64, η::Float64, norm_constrain, γ, m) where {T}
    ...
    OptParams.dw_tot .= η * OptParams.dw_tot .+ OptParams.f / sqrt(nbatch) 
    OptParams.dw_tot .*= min(1, sqrt(norm_constrain)/norm(OptParams.dw_tot))
    return OptParams.dw_tot, length(OptParams.f), res
end

function opts!(i, OptParams::OPTPARAMMINSR, optimizer::MINSRSolver, Eloc, o::Matrix{T}, nbatch, damping::Float64, dim_ps::Int64, η::Float64, norm_constrain, γ, m) where {T}
    res = norm(OptParams.f)
    lmul!(-γ, Eloc) # -delta tau * (E - E_mean)
    s = o' * o
    ...
    mul!(OptParams.f, o, OptParams.dow)
    OptParams.dw_tot .= η * OptParams.dw_tot .+ (1-η) * OptParams.f / sqrt(nbatch) 
    OptParams.dw_tot .*= min(1, sqrt(norm_constrain)/norm(OptParams.dw_tot))
    return OptParams.dw_tot, length(OptParams.f), res
end
...
function opts!(i, OptParams::OPTPARAMSKETCH, optimizer::SketchSolver, Eloc, o::Matrix{T}, nbatch, damping::Float64, dim_ps::Int64, η::Float64, norm_constrain, γ, m) where {T}
...
function opts!(i, OptParams::OPTPARAMSVD, optimizer::SVDSolver, Eloc, o::Matrix{T}, nbatch, damping::Float64, dim_ps::Int64, η::Float64, norm_constrain, γ, m) where {T}
    sr_rank0 = optimizer.sr_rank0::Int64
    sr_rank = optimizer.sr_rank::Int64
    res = norm(OptParams.f)
    lmul!(-γ, Eloc)
    ...
    if size(OptParams.u, 2) < r
        ...
        OptParams.u[:, size(OptParams.u, 2)+1:r] .= randn(mm, r - size(OptParams.u, 2))
    end
    if i == 1
