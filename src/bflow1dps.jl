...
function BFJwfTrig_lux(Nel::Integer, Pn::AbstractPoly4MLBasis, J; totdeg = length(Pn),
    ...
    BFJwf = Chain(; trans = l_trans, diff = Lux.BranchLayer(embed_layers...), to_be_prod = Lux.BranchLayer(BFwf_chain, Jastrow_chain), Sum = WrappedFunction(x -> 2 * log(abs(x[1] * x[2][1]))))
